...
class OpenImageDataset_relation(torch.utils.data.Dataset):
    ...
    def __getitem__(self,idx):
        ...
        tgt_text = f"{Label1} {item['RelationshipLabel']} {Label1}"
    ...
    def _get_all(self,idx):
        ...
        image = self.transform(image)
        loc1,loc2 = self._get_location(item)
        lb1 = f"\'loc{loc1[0]} loc{loc1[1]} loc{loc1[2]} loc{loc1[3]} {item['LabelName1']}"
        lb2 = f"\'loc{loc2[0]} loc{loc2[1]} loc{loc2[2]} loc{loc2[3]} {item['LabelName2']}"
        src_text = f"What is the relationship between {lb1} and {lb2}"
        tgt_text = f"{item['LabelName1']} {item['RelationshipLabel']} {item['LabelName2']}"
